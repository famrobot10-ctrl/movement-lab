...
public class PlayerInputState : MonoBehaviour {
 ...
 public bool JumpPressed{get;private set;} public bool JumpHeld{get;private set;} public bool CrouchPressed{get;private set;} public bool CrouchHeld{get;private set;} public bool DodgePressed{get;private set;} public bool MenuPressed{get;private set;}
 ...
 void Update(){CrouchHeld=false;JumpHeld=false;
#if ENABLE_INPUT_SYSTEM
  var gp=Gamepad.current;if(gp!=null){Move=gp.leftStick.ReadValue();Look=gp.rightStick.ReadValue();JumpPressed|=gp.buttonSouth.wasPressedThisFrame;JumpHeld|=gp.buttonSouth.isPressed;CrouchPressed|=gp.buttonEast.wasPressedThisFrame;CrouchHeld|=gp.buttonEast.isPressed;DodgePressed|=gp.leftStickButton.wasPressedThisFrame;MenuPressed|=gp.startButton.wasPressedThisFrame;}
  if(Keyboard.current!=null){Vector2 k=Vector2.zero;if(Keyboard.current.wKey.isPressed)k.y++;if(Keyboard.current.sKey.isPressed)k.y--;if(Keyboard.current.dKey.isPressed)k.x++;if(Keyboard.current.aKey.isPressed)k.x--;if(k.sqrMagnitude>0)Move=Vector2.ClampMagnitude(k,1);JumpPressed|=Keyboard.current.spaceKey.wasPressedThisFrame;JumpHeld|=Keyboard.current.spaceKey.isPressed;CrouchPressed|=Keyboard.current.cKey.wasPressedThisFrame;CrouchHeld|=Keyboard.current.cKey.isPressed;DodgePressed|=Keyboard.current.leftShiftKey.wasPressedThisFrame;MenuPressed|=Keyboard.current.escapeKey.wasPressedThisFrame;}
#endif
 }
